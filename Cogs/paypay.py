...
class PaypayCog(commands.Cog):
    ...
    @app_commands.command(name="送金リンク作成", description="PayPayの送金リンクを作成します")
    @is_allowed()
    @app_commands.describe(
        amount="送金金額（円）",
        passcode="パスコード（省略可）"
    )
    async def create_send_link(self, interaction: discord.Interaction, amount: int, passcode: str = None):
        await interaction.response.defer(ephemeral=True)

        # 金額バリデーション
        if amount <= 0:
            await interaction.followup.send(
                "❌ 金額は1円以上を指定してください。", ephemeral=True
            )
            return
        ...
        if user_id_str not in paypay_data:
            await interaction.followup.send(
                "⚠️ PayPayアカウントが未登録です。`/paypayログイン` で登録してください。",
                ephemeral=True
            )
            return
        ...
        result = await paypayu.create_link(
            user_paypay.get("phone"),
            user_paypay.get("password"),
            user_paypay.get("uuid"),
            amount,
        ...
        if result == "LOGINERR":
            await interaction.followup.send(
                "❌ PayPayログインに失敗しました。`/paypayログイン` で再登録してください。",
                ephemeral=True
            )
            return
        ...
        if not result:
            await interaction.followup.send(
                "❌ 送金リンクの作成に失敗しました。残高を確認してください。",
                ephemeral=True
            )
            return

        passcode_status = f"`{passcode}`" if passcode else "`なし`"
